...
class OneDimCNN(nn.Module):
    ...
    def __init__(
        self, max_byte_len, d_dim=256, kernel_size=[3, 4], filters=256, dropout=0.1
    ):
...
class SAM(nn.Module):
    ...
    def __init__(
        ...
        self.posembedding = nn.Embedding(
            num_embeddings=max_byte_len, embedding_dim=d_dim
        )
        ...
        self.fc = nn.Linear(in_features=256 * len(kernel_size), out_features=num_class)
    ...
    def forward(self, x, y):
        ...
        out = self.sigmoid(self.fc(out)) * 10
        return out
