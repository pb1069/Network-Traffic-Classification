...
def main():
    ...
    parser.add_argument(
        "--vocab_path", default=None, type=str, help="Path of the vocabulary file."
    )
    parser.add_argument(
        "--spm_model_path",
        default=None,
    ...
    parser.add_argument(
        "--output_model_path", type=str, required=True, help="Path of the output model."
    )
    parser.add_argument(
        "--config_path",
        type=str,
    ...
    parser.add_argument(
        "--labels_num", type=int, required=False, help="Number of prediction labels."
    )
    ...
    # Masking options.
    parser.add_argument(
        "--whole_word_masking", action="store_true", help="Whole word masking."
    )
    ...
    parser.add_argument(
        "--span_max_length", type=int, default=10, help="Max length for span masking."
    )
    ...
    if args.target == "cls":
        assert (
            args.labels_num is not None
        ), "Cls target needs the denotation of the number of labels."
    ...
    if args.world_size > 1:
        ...
        assert (
            ranks_num <= args.world_size
        ), "Started processes exceed `world_size` upper limit."
        assert (
            ranks_num <= torch.cuda.device_count()
