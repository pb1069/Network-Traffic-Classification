...
def split_cap(pcap_path, pcap_file, pcap_name, pcap_label="", dataset_level="flow"):
    ...
    if pcap_label != "":
        ...
        if not os.path.exists(
            pcap_path + "\\splitcap\\" + pcap_label + "\\" + pcap_name
        ):
            ...
            if packet_index == 0:
                ...
                if packet_direction[packet_index] != packet_direction[packet_index - 1]:
                    ...
                    for string_txt in cut(burst_data_string, int(length / 2)):
                        burst_txt += bigram_generation(
                            string_txt, packet_len=len(string_txt)
                        )
                        burst_txt += "\n"
                    burst_txt += "\n"
...
def get_feature_packet(label_pcap, payload_len):
    ...
    for packet in packets:
        ...
        packet_data_string += bigram_generation(
            new_packet_string, packet_len=payload_len, flag=True
        )
...
def get_feature_flow(label_pcap, payload_len, payload_pac):
    ...
    #     if (
    #         len(feature_result[label_pcap, extract_keys[1], extract_keys[2]].ip_lengths)
    #         < 3
    #     ):
    #         print(
    #             "preprocess flow %s but this flow has less than 3 packets." % label_pcap
    #         )
    #         return -1
    # elif len(packets) < 3:
    ...
    #             print(
    #                 "preprocess flow %s but this flow has less than 3 packets."
    #                 % label_pcap
    #             )
    #             return -1
                    ...
                    for record_index in range(source_length):
                        if extension_dict[packet_index][record_index] == "22":
                            for handshake_type_index in range(len(ms_type)):
                                extension_dict[packet_index][record_index] = (
                                    "22:" + ms_type[handshake_type_index]
                                )
                                if handshake_type_index > 0:
                                    extension_dict[packet_index].insert(
                                        handshake_type_index,
                                        ("22:" + ms_type[handshake_type_index]),
                                    )
                            break
                ...
                if len(set(extension_dict[key])) == 1 and len(extension_dict[key]) > 1:
                    ...
                    for status in extension_dict[key]:
                        if ":" in status:
                            ...
                            extension_list[key - 1] += reduce(
                                operator.mul, [int(x) for x in status.split(":")], 1
                            )
                        else:
    ...
    for packet in packets:
        ...
        if packet_count == payload_pac:
            ...
            flow_data_string += bigram_generation(
                packet_string, packet_len=payload_len, flag=True
            )
            break
        else:
            packet_data = packet.copy()
...
def generation(
    ...
    if os.path.exists(dataset_save_path + "dataset.json"):
        ...
        if clean_dataset:
            ...
            for c_k_index in range(len(change_keys)):
                ...
                new_dataset[pop_keys[c_k_index]] = new_dataset.pop(
                    change_keys[c_k_index]
                )
            ...
            os.renames(
                ...
                dataset_save_path + "\\old_dataset.json",
            )
        ...
        for r_f in target_all_files:
            if dataset_level == "flow":
                feature_data = get_feature_flow(
                    r_f, payload_len=payload_length, payload_pac=payload_packet
                )
            ...
            if len(dataset[label_id[key]]["payload"].keys()) > 0:
                dataset[label_id[key]]["payload"][
                    str(dataset[label_id[key]]["samples"])
                ] = feature_data[0]
                if dataset_level == "flow":
                    dataset[label_id[key]]["length"][
                        str(dataset[label_id[key]]["samples"])
                    ] = feature_data[1]
                    dataset[label_id[key]]["time"][
                        str(dataset[label_id[key]]["samples"])
                    ] = feature_data[2]
                    dataset[label_id[key]]["direction"][
                        str(dataset[label_id[key]]["samples"])
                    ] = feature_data[3]
                    dataset[label_id[key]]["message_type"][
                        str(dataset[label_id[key]]["samples"])
                    ] = feature_data[4]
            else:
    ...
    X, Y = obtain_data(
        pcap_path, samples, features, dataset_save_path, json_data=dataset
    )
    return X, Y
    ...
    for feature_index in range(len(features)):
        ...
        for label in json_data.keys():
            ...
            if samples[label_count] < sample_num:
                x_label = []
                for sample_index in random.sample(
                    list(json_data[label][features[feature_index]].keys()), 1500
                ):
                    x_label.append(
                        json_data[label][features[feature_index]][sample_index]
                    )
                x.append(x_label)
            else:
                x_label = []
