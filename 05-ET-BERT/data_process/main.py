...
dataset_dir = "../datasets/vpn2016/dataset/"  # the path to save dataset for dine-tuning

pcap_path = "../datasets/vpn2016/dataset/"
dataset_save_path = "../datasets/vpn2016/result/"
samples = [513, 196, 1053, 1378, 260, 332]
features = ["payload"]
dataset_level = "flow"
...
def dataset_extract(model):
    ...
    output_x_payload_train = os.path.join(
        dataset_save_path + "dataset\\", "x_datagram_train.npy"
    )

    output_x_payload_test = os.path.join(
        dataset_save_path + "dataset\\", "x_datagram_test.npy"
    )

    output_x_payload_valid = os.path.join(
        dataset_save_path + "dataset\\", "x_datagram_valid.npy"
    )
...
def models_deal(
    ...
    for index in range(len(model)):
        ...
        X_dataset[model[index]]["train"], Y_dataset[model[index]]["train"] = (
            x_train_dataset,
            y_train,
        )
        X_dataset[model[index]]["valid"], Y_dataset[model[index]]["valid"] = (
            x_valid_dataset,
            y_valid,
        )
        X_dataset[model[index]]["test"], Y_dataset[model[index]]["test"] = (
            x_test_dataset,
            y_test,
        )
...
def count_label_number(samples):
    ...
    for index in range(len(dataset_length)):
        if dataset_length[index] < samples[0]:
            print(
                "label %s has less sample's number than defined samples %d"
                % (labels[index], samples[0])
            )
