...
def evaluate(args, dataset, print_confusion_matrix=False):
    ...
    correct = 0
    precision = torch.zeros(args.labels_num).to(args.device)
    recall = torch.zeros(args.labels_num).to(args.device)
    # Confusion matrix.
    ...
    print(
        f"Accuracy: {round(accuracy,3)} F1-score: {round(f1_score.mean().item(),3)} Precision: {round(precision.mean().item(),3)} Recall: {round(recall.mean().item(),3)}"
    )
...
def main():
    ...
    print("end : Build tokenizer.")
    print("===========================")

    # Build classification model.
    print("start : build classification model.")
    model = Classifier(args)
    print("end : build classification model.")
    print("===========================")
    ...
    for epoch in tqdm.tqdm(range(1, args.epochs_num + 1)):
        ...
        for i, (src_batch, tgt_batch, seg_batch, soft_tgt_batch) in enumerate(
            ...
            if (i + 1) % args.report_steps == 0:
                ...
                total_loss = 0.0
        train_result = evaluate(args, read_dataset(args, args.train_path))
